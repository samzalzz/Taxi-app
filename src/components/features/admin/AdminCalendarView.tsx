...
export function AdminCalendarView({
  ...
  // Fetch bookings
  const { bookings: allBookings, isLoading, error } = useCalendarData(
    '/api/admin/bookings',
    dateRange
  );
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`text-xs font-medium px-3 py-1 rounded transition-colors ${
              !selectedStatus
                ? 'bg-primary text-primary-foreground'
                : 'bg-surface border border-on-surface/10 text-on-surface hover:bg-surface-light'
            }`}
          >
            Tous
          </button>
          {Object.entries(STATUS_COLORS).map(([status, config]) => (
            <button
              key={status}
              onClick={() => onStatusChange?.(status as BookingStatus)}
              className={`text-xs font-medium px-3 py-1 rounded transition-colors ${
