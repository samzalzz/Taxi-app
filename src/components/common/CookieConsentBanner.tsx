...
export function CookieConsentBanner() {
  const { hasResponded, acceptAll, rejectAll, resetConsent } = useCookieConsent();
  ...
  // Minimal footer bar once consent is given — always visible
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-surface border-t border-on-surface/10 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 py-2">
        <div className="flex items-center justify-between gap-4 text-xs text-on-surface-dim">
          <p>Vos préférences de confidentialité sont enregistrées.</p>
          <button
